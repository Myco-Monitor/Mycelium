...
class BaseApiClient:
    ...
    def _create_ssl_context(self) -> Optional[ssl.SSLContext]:
        ...
        if ca_path.exists():
            ...
            ctx.verify_mode = ssl.CERT_NONE
            self._ssl_context = ctx
            self.logger.warning(f"CA cert not found at {ca_path}, disabling SSL verification")
    ...
    async def connect(self):
        ...
        if self.session is None or self.session.closed:
            ...
            self.logger.debug(f"Created new session for {self.base_url} (TLS={self.use_tls})")
        return self.session
    ...
    async def _throttle_requests(self):
        ...
        self.request_times = [t for t in self.request_times if now - t <= self.request_period]
    ...
    def _handle_response_error(self, response: aiohttp.ClientResponse, body: str) -> ApiError:
        """
