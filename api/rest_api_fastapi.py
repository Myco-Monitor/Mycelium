...
async def get_device(device_type: str, device_id: int, api_user=Depends(require_api_key)):
...
async def get_readings(
    ...
    if device_type == 'spore':
        readings = readings_spore.get_device_readings(device_id, limit=limit, start_ts=start, end_ts=end)
    else:
        readings = readings_hyphae.get_device_readings(device_id, limit=limit, start_ts=start, end_ts=end)
...
async def get_latest_reading(device_type: str, device_id: int, api_user=Depends(require_api_key)):
...
async def acknowledge_alert(alert_id: int, request: Request, api_user=Depends(require_api_key)):
