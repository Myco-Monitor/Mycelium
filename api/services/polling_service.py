...
class PollingService:
    ...
    def __init__(self):
        ...
        try:
            with open(config_path) as f:
                config = json.load(f)
            self.polling_config = config.get('polling', {
                "spore": {
                    "interval": 60,
                    "jitter": 5,
                    "backoff_factor": 2,
    ...
    async def start(self):
        ...
        if self.polling_config["spore"]["enabled"]:
            self.polling_tasks["spore"] = asyncio.create_task(self._poll_spore_devices())
            
        if self.polling_config["hyphae"]["enabled"]:
            self.polling_tasks["hyphae"] = asyncio.create_task(self._poll_hyphae_devices())
        ...
        if self.polling_config["pressure"]["enabled"]:
            self.polling_tasks["pressure"] = asyncio.create_task(self._poll_pressure_devices())
    ...
    def configure_polling(self, device_type: str, config: Dict[str, Any]):
        ...
        self.logger.info(f"Updated polling configuration for {device_type}: {self.polling_config[device_type]}")
            ...
            try:
                ...
                for device in devices:
                    ...
                    try:
                        ...
                        reading = await self.spore_service.get_latest_reading(device_id)
                        
                        # Update device status
                        self._update_device_status("spore", device_id, True)
                        
                        self.logger.debug(f"Successfully polled Spore device {device_id}")
                    except Exception as e:
                        self.logger.error(f"Error polling Spore device {device_id}: {e}")
            ...
            try:
                ...
                for device in devices:
                    ...
                    try:
                        ...
                        # Get the latest reading
                        reading = await self.hyphae_service.get_latest_reading(device_id)
                        
                        # Update device status
                        self._update_device_status("hyphae", device_id, True)
                        
                        self.logger.debug(f"Successfully polled Hyphae device {device_id}")
                    except Exception as e:
                        self.logger.error(f"Error polling Hyphae device {device_id}: {e}")
            ...
            try:
                ...
                for location_id, location in locations.items():
                    ...
                    try:
                        ...
                        weather = await self.weather_service.get_current_weather(location_id)
                        
                        # Update device status
                        self._update_device_status("weather", location_id, True)
                        
                        self.logger.debug(f"Successfully polled weather for location {location_id}")
                    except Exception as e:
                        self.logger.error(f"Error polling weather for location {location_id}: {e}")
        ...
        while self.running:
            try:
                ...
                for device in devices:
                    ...
                    try:
                        self.logger.debug(f"Polling pressure from Hyphae device {device_id}")

                        # Fetch and store pressure reading
                        reading = await self.pressure_service.fetch_and_store_pressure(device_id)
                        ...
                        self.logger.error(f"Error polling pressure from device {device_id}: {e}")
                        self._update_device_status("pressure", device_id, False)
    ...
    def _update_device_status(self, device_type: str, device_id: str, success: bool):
        ...
        if success:
            ...
            backoff = min(
                self.polling_config[device_type]["interval"] * (backoff_factor ** status["failures"]),
                max_backoff
            )
            ...
            try:
                ...
                for trigger in triggers:
                    try:
                        ...
                        recent_alerts = alert_history.get_alerts_for_device(
                            trigger.device_id,
                            trigger.device_type,
                            days=1
                        ...
                        self.logger.info(f"Alert triggered and notification sent: {trigger.message}")
