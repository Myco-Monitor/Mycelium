...
class SporeDataService:
    ...
    async def initialize_client(self, device_id: int) -> SporeClient:
        ...
        if device_id not in self.clients:
            ...
            self.logger.info(f"Initialized client for Spore device {device['device_name']} ({device_id})")
    ...
    async def set_ambient_pressure(self, device_id: int, pressure: int) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error setting ambient pressure for device {device_id}: {e}")
    ...
    def _is_duplicate(self, device_id: int, reading: Dict[str, Any]) -> bool:
        ...
        if (
            abs(float(latest_reading["co2"]) - float(reading["co2"])) < 1 and
            abs(float(latest_reading["temp"]) - float(reading["temperature"])) < 0.1 and
            abs(float(latest_reading["humidity"]) - float(reading["humidity"])) < 0.1
        ):
            return True
