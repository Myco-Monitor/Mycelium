...
from storage.tables.relay_settings import create_relay_setting, update_relay_setting, get_device_relay_settings
from storage.tables.schedule_settings import create_schedule_setting, update_schedule_setting, get_device_schedule_settings
from storage.tables.dynamic_settings import create_dynamic_setting, update_dynamic_setting, get_device_dynamic_settings
...
class HyphaeDataService:
    ...
    async def initialize_client(self, device_id: int, pin: Optional[str] = None) -> HyphaeClient:
        """
        ...
        if device_id not in self.clients:
            base_url = f"https://{device['ip_address']}"
            client = HyphaeClient(
                base_url=base_url,
            ...
            self.logger.info(f"Initialized client for Hyphae device {device['device_name']} ({device_id})")
    ...
    async def get_client(self, device_id: int, pin: Optional[str] = None) -> HyphaeClient:
        """
    ...
    async def get_latest_reading(self, device_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error getting latest reading for device {device_id}: {e}")
    ...
    async def get_relay_config(self, device_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.error(f"Error getting relay configuration for device {device_id}: {e}")
    ...
    async def set_relay_config(self, device_id: int, config: Dict[str, Any], pin: str) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(f"Error setting relay configuration for device {device_id}: {e}")
    ...
    async def test_relay(self, device_id: int, relay_number: int, pin: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error testing relay {relay_number} for device {device_id}: {e}")
    ...
    async def get_relay_thresholds(self, device_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error getting relay thresholds for device {device_id}: {e}")
    ...
    async def set_relay_thresholds(self, device_id: int, thresholds: Dict[str, Any], pin: str) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(f"Error setting relay thresholds for device {device_id}: {e}")
    ...
    async def get_relay_schedule(self, device_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error getting relay schedule for device {device_id}: {e}")
    ...
    async def set_relay_schedule(self, device_id: int, schedule: Dict[str, Any], pin: str) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(f"Error setting relay schedule for device {device_id}: {e}")
    ...
    async def store_reading(self, device_id: int, reading: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not self._validate_reading(reading):
                self.logger.warning(f"Invalid reading for device {device_id}: {reading}")
                return None
            ...
            if self._is_duplicate(device_id, transformed_reading):
                self.logger.debug(f"Duplicate reading for device {device_id}: {transformed_reading}")
                return None
    ...
    async def store_relay_config(self, device_id: int, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for i, relay in enumerate(relays):
                ...
                existing_relay = next((r for r in existing_settings if r["relay_number"] == relay_number), None)
            ...
            self.logger.error(f"Error storing relay configuration for device {device_id}: {e}")
            return False
            
    async def store_dynamic_settings(self, device_id: int, thresholds: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            for group in groups:
                ...
                existing_group = next(
                    (g for g in existing_settings if g["group_num"] == group_num and g["parameter"] == parameter),
                    None
                )
            ...
            self.logger.error(f"Error storing dynamic settings for device {device_id}: {e}")
            return False

    async def store_schedule_settings(self, device_id: int, schedule: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            for entry in entries:
                ...
                existing_entry = next(
                    (e for e in existing_settings if e["group_num"] == group_num),
                    None
                )
            ...
            self.logger.error(f"Error storing schedule settings for device {device_id}: {e}")
            return False
    ...
    def _validate_reading(self, reading: Dict[str, Any]) -> bool:
        ...
        if not any(key in reading for key in ["temperature", "humidity", "relay_states"]):
            return False
    ...
    def _transform_reading(self, device_id: int, reading: Dict[str, Any]) -> Dict[str, Any]:
        """
