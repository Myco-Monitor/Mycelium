...
class ExportService:
    ...
    def export_readings_csv(
        ...
        output = StringIO()
        fieldnames = ['timestamp', 'device_name', 'room_name', 'co2', 'temperature', 'humidity']
    ...
    def export_harvests_csv(
        ...
        output = StringIO()
        fieldnames = ['harvest_id', 'harvest_date', 'yield_weight', 'trimmed_wt', 'bulk_name', 'room_name']
        writer = csv.DictWriter(output, fieldnames=fieldnames, extrasaction='ignore')
        writer.writeheader()
    ...
    def export_readings_excel(
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if readings:
                ...
                df = df[['timestamp', 'device_name', 'room_name', 'co2', 'temperature', 'humidity']]
            ...
            if harvests:
                ...
                harvest_df = harvest_df[['harvest_id', 'harvest_date', 'yield_weight', 'trimmed_wt', 'bulk_name', 'room_name']]
                harvest_df.to_excel(writer, sheet_name='Harvests', index=False)

            # Hourly patterns sheet
            hourly_data = self.analytics.get_hourly_pattern(start_date, end_date, room_id)
            if hourly_data:
    ...
    def export_production_report(
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            summary_data = [{
                ...
                'Total Yield (kg)': sum(h.get('yield_weight', 0) or 0 for h in harvests) if harvests else 0,
            ...
            if harvests:
                harvest_df = pd.DataFrame(harvests)
                harvest_df = harvest_df[['harvest_date', 'yield_weight', 'trimmed_wt', 'bulk_name', 'room_name']]
    ...
    def get_export_filename(self, export_type: str, start_date: str, end_date: str, extension: str = 'csv') -> str:
        """
        ...
        return f"mycelium_{export_type}_{start_date}_to_{end_date}_{timestamp}.{extension}"
