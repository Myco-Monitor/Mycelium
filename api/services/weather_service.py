...
from storage.tables.readings_weather import (
    create_reading as create_weather_data,
    get_latest_weather as get_latest_weather_data,
)
...
class WeatherDataService:
    ...
    def __init__(self):
        ...
        self._weather_cache_ttl = timedelta(
            minutes=30
    ...
    def initialize_client(
        ...
        """
        client = OpenWeatherMapClient(api_key=api_key, location=location, units=units)
    ...
    async def check_api_connection(self, location_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error checking connection for location {location_id}: {e}"
            )
            return False

    async def get_current_weather(
        self, location_id: str, use_cache: bool = True
        ...
        try:
            ...
            self.logger.error(
                f"Error getting current weather for location {location_id}: {e}"
            )
            return None

    async def get_forecast(
        self, location_id: str, days: int = 5, use_cache: bool = True
    ...
    async def get_air_pollution(
        ...
        try:
            ...
            # Transform the data
            transformed_data = self._transform_air_pollution(
                location_id, pollution_data
            )
            ...
            self.logger.error(
                f"Error getting air pollution data for location {location_id}: {e}"
            )
            return None

    async def store_weather_data(
        self, location_id: str, weather_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                db_device_id = (
                    int(location_id.split("_")[-1]) if "_" in location_id else 0
                )
            ...
            self.logger.error(
                f"Error storing weather data for location {location_id}: {e}"
            )
            raise
    ...
    def _transform_weather_data(
        ...
        # grnd_level is true ground-level pressure; OWM only includes it when
        # station data is available. Preferred over sea-level "pressure" for
        # CO2 compensation. May be absent.
        pressure_grnd = main.get("grnd_level")
    ...
    def _transform_forecast(
        self, location_id: str, forecast_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _transform_air_pollution(
        self, location_id: str, pollution_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
