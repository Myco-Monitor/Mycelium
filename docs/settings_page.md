...
- **Editable Fields:**
  ...
  - **Change Password:** self-service account password change (current + new + confirm, minimum 8 characters). The account password also confirms sensitive hub actions such as hub updates. (The former "Mycelium PIN" is retired; the `reset_pin` column remains in the DB but is unused.)
