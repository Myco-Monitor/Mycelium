                    ...
                    with ui.row().classes("items-center gap-1"):
                        for name, theme_colors in COLOR_THEMES.items():
                            ...
                            ui.element("div").classes(
                                f"mycelium-color-circle {active_class}"
                            ).style(
...
def login_page():
    ...
    with ui.column().classes("absolute-center items-center"):
        with ui.card().classes("w-96 p-6"):
            ui.label("Login to Mycelium").classes(
                "text-h5 text-center full-width q-mb-md"
            )
            ...
            ui.button("Login", on_click=handle_login).props("color=primary").classes(
                "full-width q-mb-md"
            )
...
def signup_page():
    ...
    with ui.column().classes("absolute-center items-center"):
        with ui.card().classes("w-96 p-6"):
            ui.label("Sign Up for Mycelium").classes(
                "text-h5 text-center full-width q-mb-md"
            )
            ...
            async def handle_signup():
                ...
                create_user_setting(
                    username.value.strip(), password.value, user_role=user_role
                )
                ui.navigate.to("/login")

            ui.button("Sign Up", on_click=handle_signup).props("color=primary").classes(
                "full-width q-mb-md"
            )
...
def logout_page():
    ...
    with ui.column().classes("absolute-center items-center"):
        with ui.card().classes("w-96 p-6"):
            ui.label("Logout Confirmation").classes(
                "text-h5 text-center full-width q-mb-md"
            )
            ui.label("Are you sure you want to logout?").classes(
                "text-center full-width q-mb-lg"
            )
            ...
            with ui.row().classes("full-width justify-center gap-4"):
                ...
                ui.button("Yes, Logout", on_click=confirm_logout).props(
                    "color=negative"
                )
                ui.button("Cancel", on_click=lambda: ui.navigate.to("/main")).props(
