...
def page_layout(title: str = "Mycelium"):
    ...
    with ui.header().classes("items-center justify-between px-4"):
        ...
        with ui.row().classes("items-center gap-1"):
            ...
            ui.button(icon=mode_icon, on_click=_handle_toggle_mode).props(
                "flat color=white"
            )
...
def back_to_dashboard():
    ...
    ui.button(
        "Dashboard", icon="arrow_back", on_click=lambda: ui.navigate.to("/main")
    ).props("flat color=primary").classes("q-mb-sm")
