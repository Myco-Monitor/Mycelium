...
def relay_scheduler_page():
    ...
    with ui.column().classes("w-full max-w-6xl mx-auto p-4 gap-4"):
        ...
        ui.label(
            "24-hour timeline of relay group schedules across all Hyphae devices"
        ).classes("text-muted")
        ...
        if not hyphae_devices:
            with ui.card().classes("w-full p-6 text-center"):
                ...
                ui.label(
                    "No Hyphae devices registered. Add devices in the Devices page."
                ).classes("text-muted")
            return

        device_options = {
            d["device_id"]: d.get("device_name", f"Hyphae-{d['device_id']}")
        ...
        def schedule_timeline():
            ...
            if not schedules:
                with ui.card().classes("w-full p-6 text-center"):
                    ui.label(
                        "No schedule settings configured for this device."
                    ).classes("text-muted")
                    ui.label("Configure schedules in the Devices page.").classes(
                        "text-caption text-muted"
                    )
                return

            # Build Gantt chart
            fig = _build_schedule_gantt(
                schedules, device_options.get(device_id, ""), colors
            )
            ...
            with ui.card().classes("w-full p-4 q-mt-md"):
                ...
                ui.table(columns=columns, rows=rows, row_key="group_num").classes(
                    "w-full"
                )
