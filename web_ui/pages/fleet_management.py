...
def fleet_management_page():
    ...
    with ui.column().classes('w-full max-w-6xl mx-auto p-4 gap-4'):
        ...
        ui.label('Firmware inventory, OTA updates, and device version tracking').classes('text-muted')
...
def _firmware_inventory_section(colors: dict):
    ...
    with ui.card().classes('w-full p-4 q-mb-md'):
        ...
        with ui.row().classes('w-full items-end gap-4'):
            ...
            version_input = ui.input(label='Version', placeholder='e.g., 1.2.0').classes('w-32')
            notes_input = ui.input(label='Release Notes', placeholder='Optional').classes('flex-1')
        ...
        ui.button('Save Firmware', icon='save', on_click=save_firmware).props('color=primary').classes('q-mt-sm')
    ...
    columns = [
        ...
        {'name': 'uploaded', 'label': 'Uploaded', 'field': 'uploaded_at', 'align': 'left'},
    ]
    rows = []
...
def _batch_ota_section(colors: dict):
    ...
    with ui.card().classes('w-full p-4 q-mb-md'):
        ...
        with ui.row().classes('w-full items-end gap-4'):
            dtype = ui.select(
                options=['spore', 'hyphae'], label='Device Type', value='spore',
            ).classes('w-40')
            ...
            def _fw_options():
                ...
                return {fw['version_id']: f"v{fw['version']} ({fw.get('file_size', 0) // 1024}KB)"
                        for fw in fws}

            fw_select = ui.select(
                options=_fw_options(), label='Firmware Version',
            ).classes('flex-1')
    ...
    with ui.card().classes('w-full p-4 q-mb-md'):
        ...
        ui.label('Devices with PIN status. Only devices with a PIN (device or default) can receive OTA.').classes(
        ...
        def device_checklist():
            ...
            columns = [
                ...
                {'name': 'firmware', 'label': 'Current FW', 'field': 'firmware', 'align': 'left'},
                {'name': 'pin_status', 'label': 'PIN', 'field': 'pin_status', 'align': 'center'},
            ...
            for d in devices:
                ...
                ps_label = {'device': 'Device PIN', 'default': 'Default', 'missing': 'MISSING'}[ps]
                rows.append({
                    'device_id': did,
            ...
            with device_table_container:
                ...
                table = ui.table(
                    columns=columns, rows=rows, row_key='device_id',
                    selection='multiple',
                ).classes('w-full')

                # Color code PIN status
                table.add_slot('body-cell-pin_status', r'''
                    <q-td :props="props">
    ...
    with ui.card().classes('w-full p-4'):
        ...
        batch_progress = ui.linear_progress(value=0, show_value=False).classes('w-full q-mb-sm')
        ...
        async def run_batch_ota():
            ...
            with batch_log:
                ui.label(f'Updating {total} device(s) with v{fw["version"]}...').classes('text-weight-bold')
                ...
                if not pin:
                    with batch_log:
                        with ui.row().classes('items-center gap-2'):
                            ...
                            ui.label(f'{dname} — no PIN available').classes('text-caption')
                    continue

                with batch_log:
                    status_label = ui.label(f'{dname} — uploading...').classes('text-caption')

                result = await ota_svc.upload_firmware(
                    did, dt, fw['file_path'], user_id=user_id,
                )
                ...
                with batch_log:
                    if result.get('success'):
                        ...
                        with ui.row().classes('items-center gap-2'):
                            ...
                            ui.label(f'{dname} — {result.get("error", "?")}').classes('text-caption')
        ...
        ui.button('Start Batch OTA', icon='rocket_launch',
                  on_click=run_batch_ota).props('color=primary')
...
def _device_versions_section(colors: dict):
    ...
    for d in spores:
        ...
        rows.append({
            ...
            'ip': d.get('ip_address', ''),
            'firmware': d.get('firmware_version', 'Unknown'),
            'pin_status': {'device': 'Device', 'default': 'Default', 'missing': 'MISSING'}[ps],
            'online': 'Yes' if d.get('is_online') else 'No',
        })
    for d in hyphae:
    ...
    # Color code PIN status
    table.add_slot('body-cell-pin', r'''
        <q-td :props="props">
...
def _ota_history_section(colors: dict):
    ...
    columns = [
        ...
        {'name': 'firmware', 'label': 'Firmware', 'field': 'firmware_name', 'align': 'left'},
