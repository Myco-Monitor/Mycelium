...
def _render_alert_card(
    alert: dict, alert_service: AlertService, user: dict, colors: dict, refresh_fn
):
        ...
        with ui.row().classes("w-full items-start justify-between"):
            ...
            with ui.column().classes("flex-1 gap-1"):
                with ui.row().classes("items-center gap-2"):
                    ...
                    ui.label(alert.get("rule_name", "Alert")).classes(
                        "text-weight-bold"
                    )
                    if is_resolved:
            ...
            with ui.row().classes("gap-1"):
                ui.button(
                    icon="visibility", on_click=lambda a=alert: _show_alert_detail(a)
                ).props("flat dense size=sm").tooltip("View Details")
...
def _acknowledge_alert(
    alert_id: int, alert_service: AlertService, user: dict, refresh_fn
):
...
def active_alerts_list(alert_service: AlertService, user: dict, colors: dict):
    ...
    if len(unack) > 1:
        ...
        ).props("outline color=orange").classes("q-mb-sm")

    for alert in alerts:
        _render_alert_card(
            alert, alert_service, user, colors, active_alerts_list.refresh
        )
...
def alert_history_list(alert_service: AlertService, colors: dict):
    ...
    for alert in alerts:
        _render_alert_card(
            alert, alert_service, user, colors, alert_history_list.refresh
        )
...
def alert_rules_list(alert_service: AlertService, colors: dict):
    ...
    ui.button(
        "Add Alert Rule",
        icon="add",
        on_click=lambda: _open_add_rule_dialog(alert_service, colors),
    ...
    with ui.card().classes("w-full p-3"):
        with ui.row().classes("w-full items-center justify-between"):
            ...
            with ui.row().classes("gap-1"):
                ui.button(
                    icon="edit",
                    on_click=lambda r=rule: _open_edit_rule_dialog(
                        r, alert_service, colors
                    ),
