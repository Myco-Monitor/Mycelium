...
from storage.tables.grow_rooms import (
    ...
    create_grow_room,
    update_grow_room,
    deactivate_grow_room,
    reactivate_grow_room,
)
    ...
    with ui.column().classes("w-full max-w-6xl mx-auto p-4 gap-4"):
        ...
        with ui.row().classes("w-full items-center justify-between"):
            with ui.column().classes("gap-0"):
                ...
                ui.label("Manage farms, rooms, and view device assignments").classes(
                    "text-muted"
                )
            ui.button(
                "Add Farm", icon="add", on_click=lambda: _open_add_farm_dialog(colors)
            ).props("color=primary")
...
def farm_list(colors: dict):
    ...
    if not farms:
        with ui.card().classes("w-full p-6 text-center"):
            ...
            ui.label('No farms yet. Click "Add Farm" to get started.').classes(
                "text-muted"
            )
        return
    ...
    with ui.card().classes("w-full").style(opacity):
        ...
        with ui.row().classes("w-full items-center px-4 pt-4 pb-2"):
            ...
            with ui.column().classes("flex-grow gap-0"):
                ...
                ui.label(farm.get("farm_loc", "") or "No location").classes(
            ...
            with ui.row().classes("gap-1"):
                ui.button(
                    icon="edit",
                    on_click=lambda f=farm: _open_edit_farm_dialog(f, colors),
                ).props("flat dense color=primary size=sm")
                if is_active:
                    ui.button(
                        icon="archive",
                        on_click=lambda f=farm: _confirm_deactivate_farm(f, colors),
                    ).props("flat dense color=negative size=sm").tooltip("Deactivate")
                else:
                    ui.button(
                        icon="unarchive", on_click=lambda f=farm: _reactivate_farm(f)
                    ).props("flat dense color=positive size=sm").tooltip("Reactivate")
...
def _room_section(farm_id: int, colors: dict):
    ...
    with ui.row().classes("w-full justify-end q-mb-sm"):
        ui.button(
            "Add Room",
            icon="add",
            on_click=lambda: _open_add_room_dialog(farm_id, colors),
    ...
    with ui.card().classes("w-full p-3").style(opacity):
        with ui.row().classes("w-full items-center"):
            ...
            with ui.row().classes("gap-1"):
                ui.button(
                    icon="edit",
                    on_click=lambda r=room: _open_edit_room_dialog(r, colors),
                ).props("flat dense size=sm color=primary")
                if is_active:
                    ui.button(
                        icon="archive",
                        on_click=lambda r=room: _confirm_deactivate_room(r, colors),
                    ).props("flat dense size=sm color=negative").tooltip("Deactivate")
                else:
                    ui.button(
                        icon="unarchive", on_click=lambda r=room: _reactivate_room(r)
                    ).props("flat dense size=sm color=positive").tooltip("Reactivate")
...
def _open_add_farm_dialog(colors: dict):
    with ui.dialog() as dlg, ui.card().classes("p-4 min-w-96"):
        ...
        name = ui.input(label="Farm Name", placeholder="e.g., Main Farm").classes(
            "w-full"
        )
        loc = ui.input(label="Location", placeholder="e.g., 123 Farm Road").classes(
            "w-full"
        )
...
def _open_edit_farm_dialog(farm: dict, colors: dict):
    with ui.dialog() as dlg, ui.card().classes("p-4 min-w-96"):
        ...
        name = ui.input(label="Farm Name", value=farm.get("farm_name", "")).classes(
            "w-full"
        )
        loc = ui.input(label="Location", value=farm.get("farm_loc", "")).classes(
            "w-full"
        )
        desc = ui.textarea(
            label="Description", value=farm.get("farm_desc", "")
        ).classes("w-full")
...
def _confirm_deactivate_farm(farm: dict, colors: dict):
    with ui.dialog() as dlg, ui.card().classes("p-4"):
        ...
        ui.label("The farm and its rooms will be hidden but not deleted.").classes(
            "text-muted q-mb-md"
        )
...
def _open_add_room_dialog(farm_id: int, colors: dict):
    with ui.dialog() as dlg, ui.card().classes("p-4 min-w-96"):
        ...
        name = ui.input(
            label="Room Name", placeholder="e.g., Fruiting Chamber A"
        ).classes("w-full")
        desc = ui.textarea(label="Description (optional)").classes("w-full")
...
def _open_edit_room_dialog(room: dict, colors: dict):
    with ui.dialog() as dlg, ui.card().classes("p-4 min-w-96"):
        ...
        name = ui.input(label="Room Name", value=room.get("room_name", "")).classes(
            "w-full"
        )
        desc = ui.textarea(
            label="Description", value=room.get("room_desc", "")
        ).classes("w-full")
...
def _confirm_deactivate_room(room: dict, colors: dict):
    with ui.dialog() as dlg, ui.card().classes("p-4"):
        ...
        ui.label("Devices in this room will remain but become unassigned.").classes(
            "text-muted q-mb-md"
        )
        with ui.row().classes("w-full justify-end gap-2"):
            ui.button("Cancel", on_click=dlg.close).props("outline")
