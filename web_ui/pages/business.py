    ...
    with ui.column().classes('w-full max-w-6xl mx-auto p-4 gap-4'):
        ...
        with ui.card().classes('w-full p-4'):
            ...
            with ui.row().classes('w-full gap-4 flex-wrap q-mb-md'):
                ...
                _kpi_card('inventory_2', 'Substrate On Hand', '0', 'Substrate Ready', colors)
            ...
            with ui.row().classes('w-full gap-4 flex-wrap'):
                ...
                _kpi_card('trending_up', 'Net Profit', '$0', 'Revenue - Expenses', colors)
        ...
        with ui.row().classes('w-full gap-4 flex-wrap'):
            with ui.card().classes('p-4 flex-1 min-w-72'):
                ...
                ui.label('Recent sales, harvests, and alerts will appear here...').classes('text-muted text-center')
            ...
            with ui.card().classes('p-4 flex-1 min-w-72'):
                ...
                with ui.row().classes('w-full gap-4 q-mb-md'):
                    with ui.column().classes('flex-1 items-center gap-0'):
                        ui.label('Production Efficiency').classes('text-subtitle2 text-weight-bold')
                        ui.label('Yield per batch tracking').classes('text-caption text-muted')
                    with ui.column().classes('flex-1 items-center gap-0'):
                        ui.label('Labour Productivity').classes('text-subtitle2 text-weight-bold')
                        ui.label('Revenue per hour worked').classes('text-caption text-muted')
                with ui.row().classes('w-full gap-4 q-mb-md'):
                    with ui.column().classes('flex-1 items-center gap-0'):
                        ui.label('Inventory Turnover').classes('text-subtitle2 text-weight-bold')
                        ui.label('Days of stock on hand').classes('text-caption text-muted')
                    with ui.column().classes('flex-1 items-center gap-0'):
                        ui.label('Customer Analysis').classes('text-subtitle2 text-weight-bold')
                        ui.label('Top customers by revenue').classes('text-caption text-muted')

                ui.separator().classes('q-my-sm')
                ui.label('System Alerts').classes('text-subtitle1 text-weight-bold text-center q-mb-sm')
                ui.label('Low inventory warnings and overdue bills will appear here...').classes('text-muted text-center text-caption')
...
def _mgmt_card(icon: str, title: str, description: str, button_label: str, colors: dict, href: str = None):
        ...
        with ui.column().classes('items-center gap-2 w-full'):
            ...
            if href:
                ui.button(button_label,
                          on_click=lambda h=href: ui.navigate.to(h)).classes('w-full q-mt-sm')
            else:
                ui.button(button_label,
                          on_click=lambda: ui.notify('Coming soon', type='info')).classes('w-full q-mt-sm')


def _quick_button(icon: str, label: str, colors: dict):
    """Render a quick-action button."""
    ui.button(label, icon=icon,
              on_click=lambda: ui.notify('Coming soon', type='info'))
