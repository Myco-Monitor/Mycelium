...
def dashboard_page():
    ...
    with ui.column().classes('w-full max-w-6xl mx-auto p-4 gap-4'):
        ...
        with ui.element('div').style(
            ...
            _nav_card('Devices', 'Manage and monitor all connected devices',
                       'sensors', '/devices', colors)
            _nav_card('Farm Overview', 'View farms, rooms, and device health',
                       'agriculture', '/farms', colors)
            _nav_card('Alerts', 'Configure alert rules and view history',
                       'notifications', '/alerts', colors)
            _nav_card('Analytics', 'Analyze historical sensor data',
                       'analytics', '/analytics', colors)
            _nav_card('Business', 'Track spawn, harvest, and sales',
                       'business', '/business', colors)
            _nav_card('Settings', 'User preferences and configuration',
...
def _nav_card(title: str, description: str, icon: str, href: str, colors: dict):
    ...
    with ui.card().classes('p-4 cursor-pointer').style('height: 100%;').on(
        'click', lambda href=href: ui.navigate.to(href)
    ):
