...
def _build_env_trends_chart(readings, colors):
    ...
    fig.add_trace(go.Scatter(
        x=timestamps, y=co2, name='CO2 (ppm)',
        yaxis='y', line=dict(color='#ef5350')
    ))
    fig.add_trace(go.Scatter(
        x=timestamps, y=temp, name='Temp (C)',
        yaxis='y2', line=dict(color='#42a5f5')
    ))
    fig.add_trace(go.Scatter(
        x=timestamps, y=humidity, name='Humidity (%)',
        yaxis='y3', line=dict(color='#66bb6a')
    ))
    ...
    fig.update_layout(
        ...
        yaxis3=dict(title='Humidity (%)', side='right', overlaying='y',
...
def _build_hourly_chart(hourly_data, field, title, color):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=hour_labels, y=values, name=title,
...
def analytics_page():
    ...
    default_start = (
        (datetime.strptime(default_end, '%Y-%m-%d') - timedelta(days=30)).strftime('%Y-%m-%d')
    )
    ...
    with ui.card().classes('w-full p-3'):
        with ui.row().classes('w-full items-end gap-3 flex-wrap'):
            ...
            room_select = ui.select(
                room_options, value=None, label='Room'
            ).classes('w-40')
            ...
            ui.button('Export CSV', icon='download', on_click=_export_csv).props('dense outline')
...
def _build_env_trends(stats, readings, colors):
    ...
    with ui.row().classes('w-full gap-3 flex-wrap q-mb-md'):
        if stats:
            ...
            _stat_card('Avg Humidity', f'{stats.humidity_mean:.1f}%', 'water_drop', colors)
    ...
    with ui.card().classes('w-full p-3'):
        ui.label('Environmental Trends').classes('text-subtitle1 text-weight-bold q-mb-sm')
...
def _build_daily_patterns(hourly, colors):
    ...
    with ui.row().classes('w-full gap-3 flex-wrap'):
        with ui.card().classes('flex-1 min-w-72 p-3'):
            ui.label('CO2 Hourly Pattern').classes('text-subtitle1 text-weight-bold q-mb-sm')
            fig = _build_hourly_chart(hourly, 'avg_co2', 'CO2 (ppm)', '#ef5350')
            ui.plotly(fig).classes('w-full').style('height: 300px')

        with ui.card().classes('flex-1 min-w-72 p-3'):
            ui.label('Temperature Hourly Pattern').classes('text-subtitle1 text-weight-bold q-mb-sm')
            fig = _build_hourly_chart(hourly, 'avg_temp', 'Temp (C)', '#42a5f5')
            ui.plotly(fig).classes('w-full').style('height: 300px')

        with ui.card().classes('flex-1 min-w-72 p-3'):
            ui.label('Humidity Hourly Pattern').classes('text-subtitle1 text-weight-bold q-mb-sm')
...
def _build_insights(insights, colors):
    ...
    if not insights:
        with ui.card().classes('w-full p-4'):
            ui.label('No insights available for the selected period.').classes('text-muted')
            ui.label(
            ...
            with ui.row().classes('items-start gap-3'):
                ...
                with ui.column().classes('gap-1 flex-1'):
                    ...
                    with ui.row().classes('items-center gap-2 q-mt-xs'):
                        ui.badge(insight.metric.upper()).props(f'color={badge_type} outline')
...
def _build_notebook_panel(colors):
    ...
    with ui.card().classes('w-full p-3'):
        with ui.row().classes('gap-2 flex-wrap'):
            ...
            ui.button('Run All Cells', icon='play_arrow', on_click=_run_all).props('dense')
            ...
            ui.button('Save Notebook', icon='save', on_click=_save_nb).props('dense outline')
    ...
    with ui.card().classes('w-full p-4 q-mt-md'):
        ui.label('Database Quick Reference').classes('text-subtitle1 text-weight-bold q-mb-sm')
        ui.label('Common functions and key tables for querying the Mycelium database.').classes(
            'text-caption text-muted q-mb-md'
        )

        with ui.row().classes('w-full gap-4 flex-wrap'):
            with ui.column().classes('flex-1 min-w-64'):
                ui.label('Key Tables').classes('text-subtitle2 text-weight-bold q-mb-xs')
                _ref_items([
                    ('readings_spore', 'Sensor readings (co2, temp, humidity, reading_ts)'),
                    ('device_spore', 'Spore devices (device_id, device_name, room_id)'),
                    ('device_hyphae', 'Hyphae controllers (device_id, device_name)'),
                    ('grow_rooms', 'Grow rooms (room_id, room_name, farm_id)'),
                    ('harvest', 'Harvest records (harvest_id, harvest_ts, total_wt)'),
                    ('bulk', 'Bulk substrates (bulk_id, bulk_name, room_id)'),
                ])

            with ui.column().classes('flex-1 min-w-64'):
                ui.label('Common Queries').classes('text-subtitle2 text-weight-bold q-mb-xs')
                _ref_code_items([
            ...
            with ui.column().classes('flex-1 min-w-64'):
                ui.label('Service Functions').classes('text-subtitle2 text-weight-bold q-mb-xs')
                _ref_items([
                    ('analytics.get_readings_for_period(start, end, room_id)', 'Fetch readings'),
                    ('analytics.calculate_environmental_stats(readings)', 'Compute stats'),
                    ('analytics.get_harvests_for_period(start, end, room_id)', 'Fetch harvests'),
                    ('analytics.get_hourly_pattern(start, end, room_id)', 'Hourly averages'),
                    ('analytics.generate_insights(start, end, room_id)', 'Auto insights'),
                ])
...
def _render_cell(cell, idx, cells, refresh_fn):
    ...
    with ui.card().classes('w-full q-mb-sm'):
        with ui.row().classes('items-center justify-between p-2'):
            ui.label(f'Cell {idx + 1}').classes('text-caption text-weight-bold text-muted')
            with ui.row().classes('gap-1'):
        ...
        # Output area
        if cell.get('output'):
            with ui.card().classes('w-full q-mt-xs p-2').style(
                'background-color: rgba(0,0,0,0.15); border-left: 3px solid grey;'
            ):
                ui.label('Output:').classes('text-caption text-muted')
                ui.html(f'<pre style="white-space: pre-wrap; margin: 0; font-family: monospace;">'
                        f'{cell["output"]}</pre>')
...
def _ref_items(items):
    ...
    for name, desc in items:
        with ui.row().classes('items-start gap-2 q-mb-xs'):
            ui.label(name).classes('text-caption text-weight-bold').style('font-family: monospace')
