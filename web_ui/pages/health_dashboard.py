    ...
    with ui.column().classes('w-full max-w-6xl mx-auto p-4 gap-4'):
        with ui.row().classes('w-full items-center justify-between'):
            with ui.column().classes('gap-0'):
                ...
                ui.label('Real-time health monitoring for all devices').classes('text-muted')
            ui.button('Refresh', icon='refresh', on_click=lambda: (
                _health_summary.refresh(), health_grid.refresh()
            )).props('outline')
...
def _health_summary(colors: dict):
    ...
    with ui.row().classes('w-full gap-4 flex-wrap'):
        ...
        _stat_card('Online', str(online), 'check_circle', colors, color=STATUS_COLORS['online'])
        _stat_card('Offline', str(offline), 'error', colors,
                   color=STATUS_COLORS['offline'] if offline > 0 else colors.get('text_muted', '#999'))
...
def health_grid():
    ...
    from storage.tables.device_health import (
        get_all_devices_health_summary, get_device_health_metrics,
    )
    ...
    for device, dtype in all_devices:
        ...
        try:
            metrics_data[f"{dtype}:{device_id}"] = get_device_health_metrics(device_id, dtype)
        except Exception:
            pass
...
def _device_health_card(device: dict, dtype: str, health_data: dict,
                        metrics_data: dict, colors: dict):
    ...
    avg_response = metrics.get('avg_response_time_ms', health.get('avg_response_time_ms'))
    ...
    with ui.card().classes('p-4'):
        ...
        with ui.row().classes('w-full items-center justify-between q-mb-sm'):
            ...
            ui.badge(status_text).style(f'background-color: {status_color}; color: white')
        ...
        bar_color = 'green' if bar_value > 0.9 else 'orange' if bar_value > 0.5 else 'red'
