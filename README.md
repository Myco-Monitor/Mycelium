...
### Analytics
- **Reports** — how did the period go: pick a room, Spore or Sentinel set and a date range (whole days in your time zone) and get a fixed summary: per-device averages and extremes; **Compliance**, the share of samples inside *your own* alert thresholds with excursion counts and the longest excursion (a metric with no rule says so, there are no built-in targets); **Equipment**, hours and percent on for each relay of the room's Hyphae, excluding test pulses and offline gaps; **Alerts** triggered in the period; a **Daily summary** table of min / average / max per device per day with relay hours and a CSV download; then three insight cards: **Control response**, how the served Spores' CO2 / humidity / temperature move in the half hour after each relay of the room's Hyphae switches on (superposed-epoch curves per relay, time to half effect, recovery after off, no-response counts, and daily relay hours against daily means); **Room comparison**, the room's Spore mean minus a baseline Sentinel of your choice as hour-of-day × date heatmaps plus the lag at which room CO2 best follows the baseline; and **Sensor agreement**, the rolling difference between each pair of Spores in a room against a band you set, its drift per day, and per-device stretches that stray from the device's own hour-of-day pattern. Every card says in one line why it is not available when the selection lacks its inputs. Harvest Analysis charts yields from the harvest table.
- **Explore** — what exactly happened: free-form charts over any readings table (Spore, Sentinel, weather, Hyphae relay, pressure): pick a device, metrics and a chart type; up to three y-axes for mixed units. A Spore linked to a Hyphae gets that Hyphae's relay ON stretches shaded behind its lines, one legend entry per relay. No code execution.
