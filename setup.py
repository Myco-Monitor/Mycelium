...
def prompt_environment_choice():
    ...
    print("=" * 40)
    print("To avoid conflicts with your system Python, we'll create an isolated environment.")
    print("\nAvailable options:")
...
def create_requirements_file():
    ...
    requirements = [
        ...
        # Analytics and Jupyter Integration
        "jupyter>=1.0.0",
        "jupyterlab>=4.0.0",
        "notebook>=7.0.0",
        "matplotlib>=3.5.0",
...
def initialize_database(reset=False, sample_data=False):
    ...
    logs_dir = project_root / "web_app" / "logs"
...
def create_sample_data():
    ...
    try:
        ...
        # Sample spawn batches - using direct database insertion since business_manager has different parameters
        import sqlite3
        db_path = project_root / "data" / "mycelium.db"
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        
        # Sample spawn data for direct insertion
        spawn_data = [
            ('SP001', 'Oyster Mushroom', 'Premium Spawn Co', 5.0, 12.50, 
             (datetime.now() - timedelta(days=30)).isoformat(), 14, 'High quality spawn batch'),
            ('SP002', 'Shiitake', 'Premium Spawn Co', 3.0, 18.00,
             (datetime.now() - timedelta(days=25)).isoformat(), 21, 'Premium shiitake strain')
        ]
        
        # Insert spawn data directly
        cursor.executemany('''
            INSERT OR REPLACE INTO spawn 
            (batch_id, strain, supplier, quantity_kg, cost_per_kg, inoculation_date, expected_colonization_days, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', spawn_data)
        ...
        customer_data = [
            {
                ...
                'phone': '555-0123',
                'address': '123 Market St, Farmville, ST 12345',
                'customer_type': 'Retail',
                'notes': 'Weekly orders, prefers organic'
            },
            {
                'name': 'Farm Fresh Restaurant',
                'email': 'chef@farmfresh.com',
                'phone': '555-0456',
                'address': '456 Main St, Foodtown, ST 12346',
                'customer_type': 'Restaurant',
                'notes': 'High-end restaurant, quality focused'
            }
        ]
        
        for customer in customer_data:
            business_manager.create_customer(customer)
        
        # Sample employees
        employee_data = [
            {
                'name': 'John Smith',
                'email': 'john@myceliumfarm.com',
                'phone': '555-0789',
                'position': 'Farm Manager',
                'hourly_rate': 25.00,
                'hire_date': (datetime.now() - timedelta(days=365)).isoformat(),
                'notes': 'Experienced mushroom cultivation specialist'
            },
            {
                'name': 'Sarah Johnson',
                'email': 'sarah@myceliumfarm.com',
                'phone': '555-0987',
        ...
        for employee in employee_data:
            business_manager.create_employee(employee)
...
def validate_installation():
    ...
    required_modules = [
        ...
        'pandas', 'numpy', 'flask_login'
    ]
    
    # Check analytics modules
    analytics_modules = [
        'jupyter', 'matplotlib', 'seaborn', 'sklearn', 'scipy'
    ]
...
def main():
    ...
    parser.add_argument("--env-type", choices=['venv', 'conda', 'existing'],
                       help="Environment type: 'venv' (virtualenv), 'conda', or 'existing' (use current)")
    parser.add_argument("--env-name", default="mycelium",
                       help="Environment name (default: 'mycelium')")
    ...
    if not initialize_database(reset=args.reset_db, sample_data=args.sample_data):
