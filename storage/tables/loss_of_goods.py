...
def create_loss_of_goods(item_type: str, source_type: str, loss_date: str, quantity: float,
                        farm_id: Optional[int] = None, source_id: Optional[int] = None,
                        reason: Optional[str] = None, notes: Optional[str] = None) -> int:
    """
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """
    return execute_insert(query, (farm_id, item_type, source_id, source_type, loss_date, quantity, reason, notes))
...
def get_loss_of_goods_by_source(source_id: int, source_type: str) -> List[Dict[str, Any]]:
    """
...
def get_loss_of_goods_by_date_range(start_date: str, end_date: str) -> List[Dict[str, Any]]:
    """
...
def get_loss_of_goods_by_farm_and_date_range(farm_id: int, start_date: str, end_date: str) -> List[Dict[str, Any]]:
    """
...
def calculate_total_loss_by_item_type(item_type: str, start_date: Optional[str] = None, 
    ...
    if start_date and end_date:
        ...
        query = "SELECT SUM(quantity) as total_loss FROM loss_of_goods WHERE item_type = ?"
        result = execute_query(query, (item_type,))
    
    return result[0]['total_loss'] if result and result[0]['total_loss'] is not None else 0.0

def calculate_total_loss_by_farm(farm_id: int, start_date: Optional[str] = None, 
                               end_date: Optional[str] = None) -> float:
    """
    ...
    if start_date and end_date:
        ...
        query = "SELECT SUM(quantity) as total_loss FROM loss_of_goods WHERE farm_id = ?"
        result = execute_query(query, (farm_id,))
    
    return result[0]['total_loss'] if result and result[0]['total_loss'] is not None else 0.0

def update_loss_of_goods(loss_id: int, item_type: Optional[str] = None,
                        source_type: Optional[str] = None, loss_date: Optional[str] = None,
                        quantity: Optional[float] = None, reason: Optional[str] = None,
