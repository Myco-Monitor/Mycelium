...
def create_cost_of_goods(item_id: int, item_name: str, item_cost: float, item_count: int, 
                        weight_lbs: float, item_used: int = 0, used_weight: float = 0.0,
                        purchase_ts: Optional[str] = None) -> int:
    """
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """
    return execute_insert(query, (item_id, item_name, item_cost, item_count, weight_lbs, 
                                 item_used, used_weight, purchase_ts))
...
def get_cost_of_goods_by_name(item_name: str) -> List[Dict[str, Any]]:
    ...
    """
    query = "SELECT * FROM cost_of_goods WHERE item_name LIKE ? ORDER BY purchase_ts DESC"
    return execute_query(query, (f"%{item_name}%",))

def get_cost_of_goods_by_date_range(start_date: str, end_date: Optional[str] = None) -> List[Dict[str, Any]]:
    """
...
def update_cost_of_goods(unit_id: int, item_id: Optional[int] = None, item_name: Optional[str] = None, 
                        item_cost: Optional[float] = None, item_count: Optional[int] = None,
                        weight_lbs: Optional[float] = None, item_used: Optional[int] = None,
                        used_weight: Optional[float] = None, purchase_ts: Optional[str] = None) -> int:
    """
...
def update_usage(unit_id: int, additional_used: int = 0, additional_weight: float = 0.0) -> int:
    """
