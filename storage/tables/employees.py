...
def create_employee(
    ...
    return execute_insert(
        query, (farm_id, emp_name, emp_role, emp_rate, emp_contact, emp_start)
    )


def get_employee(
    emp_id: int, include_inactive: bool = False
    ...
    if include_inactive:
        ...
        return (
            execute_query(query, (emp_id,))[0]
            if execute_query(query, (emp_id,))
            else None
        )
    else:
...
def get_employees_by_farm(
    farm_id: int, include_inactive: bool = False
) -> List[Dict[str, Any]]:
    """
    ...
    if include_inactive:
        ...
        query = (
            "SELECT * FROM employees WHERE farm_id = ? AND active = 1 ORDER BY emp_name"
        )
        return execute_query(query, (farm_id,))


def get_employees_by_role(
    emp_role: str, include_inactive: bool = False
) -> List[Dict[str, Any]]:
    """
...
def search_employees(
    search_term: str, include_inactive: bool = False
) -> List[Dict[str, Any]]:
    """
