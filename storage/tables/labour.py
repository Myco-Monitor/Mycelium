...
def create_labour(emp_id: int, task_type: str, work_date: str, hours_worked: float, 
...
def get_labour_by_date_range(start_date: str, end_date: str) -> List[Dict[str, Any]]:
    ...
    """
    query = "SELECT * FROM labour WHERE work_date BETWEEN ? AND ? ORDER BY work_date DESC"
    return execute_query(query, (start_date, end_date))

def get_labour_by_employee_and_date_range(emp_id: int, start_date: str, end_date: str) -> List[Dict[str, Any]]:
    """
...
def calculate_total_hours_by_employee(emp_id: int, start_date: Optional[str] = None, 
    ...
    return result[0]['total_hours'] if result and result[0]['total_hours'] is not None else 0.0

def calculate_total_hours_by_task_type(task_type: str, start_date: Optional[str] = None, 
                                      end_date: Optional[str] = None) -> float:
    """
    ...
    if start_date and end_date:
        ...
        query = "SELECT SUM(hours_worked) as total_hours FROM labour WHERE task_type = ?"
        result = execute_query(query, (task_type,))
    
    return result[0]['total_hours'] if result and result[0]['total_hours'] is not None else 0.0

def update_labour(labour_id: int, task_type: Optional[str] = None,
                 work_date: Optional[str] = None, hours_worked: Optional[float] = None,
                 notes: Optional[str] = None) -> int:
    """
