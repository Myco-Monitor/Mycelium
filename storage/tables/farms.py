...
def create_farm(farm_name: str, farm_loc: Optional[str] = None, 
                farm_desc: Optional[str] = None) -> int:
    """
...
def update_farm(farm_id: int, farm_name: Optional[str] = None, 
                farm_loc: Optional[str] = None, farm_desc: Optional[str] = None) -> int:
    """
...
def get_farm_statistics(farm_id: int) -> Dict[str, Any]:
    ...
    """
    # Get room count
    room_query = "SELECT COUNT(*) as count FROM grow_rooms WHERE farm_id = ? AND active = 1"
